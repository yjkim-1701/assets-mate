...
## 2. 자산 목록 (mockup 데이터 기준)

### 2.1 `mockup/src/data/mock.ts` — `ASSETS`
...
mockup에서 실제 미디어를 쓰게 될 때는 예를 들어 `mockup/public/sample/` 아래에 두고, 빌드 경로는 `/sample/...`로 참조할 수 있다. 본 문서는 **소스·프롬프트 정리**만 담당하며, 코드 연동은 별도 작업이다.
